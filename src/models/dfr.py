from models.feat_cae import FeatCAE
import torch
...
class DFR(nn.Module):
    def __init__(self, backbone='vgg19',
        ...
        # feature extractor
        self.extractor = Extractor(backbone=self.backbone,
        ...
        if self.in_channels is None or self.latent_dim is None:
            if self.data_loader is None:
                raise Exception('if in_channels or latent_dim are None data_loader must be defined!!!')
            self.estimate_parameters()

        # autoencoder
        self.autoencoder = FeatCAE(width=featmap_size[0]//4, height=featmap_size[0]//4, in_channels=self.in_channels,
                                   latent_dim=self.latent_dim,
                                   is_bn=self.is_bn).to(self.device)
